...
router.put(
    ...
    body('users')
        .isArray({ min: 1 })
        .withMessage(
            'Users must be an array of user IDs'
        ),

    body('users.*')
        .notEmpty()
        .withMessage(
            'Each user ID is required'
        )
        .isMongoId()
        .withMessage(
            'Each user must be a valid MongoDB ObjectId'
        ),
...
router.get(
    ...
    async (req, res) => {
        ...
        try {
            ...
            // Demo user can only count AI messages
            // from demo projects.
...
router.post(
    ...
    body('userIds.*')
        .isMongoId()
        .withMessage(
            'Each userId must be a valid MongoDB ObjectId'
        ),
