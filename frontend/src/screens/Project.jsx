...
const FileTreeNode = ({
                    ...
                    >
                        ...
                        <i
                            className={`text-sm shrink-0 ${
                                isOpen
                                    ? 'ri-folder-open-line text-amber-400'
                                    : 'ri-folder-line text-amber-400'
                            }`}
                        ></i>
                    ...
                    {hovering && (
                        <div className='flex items-center gap-0.5 shrink-0'>
                            ...
                            <button
                                ...
                                title='Delete'
                                className='p-1 rounded hover:bg-red-900/40 text-slate-400 hover:text-red-400 transition'
                            >
    ...
    return (
        <div
            ...
            onMouseLeave={() => setHovering(false)}
            style={{ paddingLeft: `${depth * 12 + 4}px` }}
            className={`flex items-center group pr-1 hover:bg-slate-700/60 transition ${
                currentFile === path
        ...
        >
            ...
            >
                <i
                    className={`text-sm shrink-0 ${extIcon(name)}`}
                ></i>
            ...
            {hovering && (
                <button
                    ...
                    title='Delete'
                    className='p-1 rounded hover:bg-red-900/40 text-slate-400 hover:text-red-400 transition shrink-0'
                >
                    <i className='ri-delete-bin-6-line text-xs'></i>
                </button>
            )}
...
const Project = () => {
    ...
    useEffect(() => {
        if (newItemDialog && newItemInputRef.current) {
    ...
    const toWebContainerTree = (tree) => {
        ...
        for (const [name, node] of Object.entries(tree)) {
            if (node && typeof node === 'object' && node.file) {
                ...
                result[name] = {
                    directory: toWebContainerTree(node)
                };
            }
        }
    ...
    const confirmNewItem = () => {
        ...
        if (newItemDialog.parentPath === null) {
            ...
            const pathParts =
                newItemDialog.parentPath.split('/');

            const parentNode =
                getNodeByPath(fileTree, pathParts);
    ...
    const handleDelete = (path, isFolder) => {
        if (
            !window.confirm(
                `Delete ${isFolder ? 'folder' : 'file'} "${path
                    .split('/')
                    .pop()}"?`
            )
        ) {
    ...
    const handleDrag = (e) => {
        ...
        if (
            isDraggingTerminal.current &&
            codeColumnRef.current
        ) {
            const rect =
                codeColumnRef.current.getBoundingClientRect();
    ...
    function inviteCollaborators() {
        toast.promise(
            axios.post(
                ...
                {
                    projectId,
                    userIds: Array.from(selectedUserId)
                }
            ).then(res => {
                ...
                if (
                    ...
                    toast.info(
                        'Selected user(s) already invited or added'
                    );
                } else if (skipped.length > 0) {
                    toast.info(
                        `${invited.length} invite(s) sent, ${skipped.length} skipped`
                    );
                }
            }),
            {
    ...
    useEffect(() => {
        ...
        getWebContainer()
            ...
            .catch(err => {
                console.error(
                    'WebContainer initialization error:',
                    err
                );
            });
        ...
        receiveMessage(
            ...
            data => {
                ...
                if (
                    withTimeStamp?.sender?._id ===
                        'ai' &&
                    withTimeStamp?.message
                ) {
                    try {
                        const parsed =
                            typeof withTimeStamp.message ===
                            'string'
                                ? JSON.parse(
                                      withTimeStamp.message
                                  )
    ...
    const autoInstall = async ft => {
        if (!webContainerRef.current) {
            toast.error(
                "Container not ready – cannot auto-install."
            );
        ...
        if (!isValid) {
            addLog(
                '⚠️ Invalid file tree received, skipping mount.',
                'error'
            );
        ...
        try {
            ...
            const installProcess =
                await webContainerRef.current.spawn(
                    "npm",
                    ["install"]
                );
            ...
            addLog(
                '✓ Packages installed! Click Run to start the server.',
                'success'
            );
        } catch (err) {
            addLog(
    ...
    const handleRun = async () => {
        ...
        try {
            // Always mount the latest editor contents
            addLog(
            ...
            if (
                ...
                addLog(
                    `🐍 Python detected: ${pythonFiles[0]}`,
                    'info'
                );

                addLog(
                    '⚠️ Python execution is not supported by WebContainer.',
                    'error'
                );

                addLog(
                    'Use JavaScript/Node.js for WebContainer execution, or add a separate backend code-execution service for Python.',
                    'info'
                );
                ...
                tempRunProcess.exit.then(
                    exitCode => {
                        if (
                            ...
                            addLog(
                                `❌ Server stopped with exit code ${exitCode}.`,
                                'error'
                            );
                        }
            ...
            if (jsFiles.length > 0) {
                /*
                 * Prefer the currently selected JS file.
                 *
                 * Example:
                 * calculator.js
                 *
                 * If calculator.js is open in editor,
                 * Run will execute calculator.js.
                 */
                let jsFile = null;
                ...
                const exitCode =
                    await tempRunProcess.exit;

                if (exitCode === 0) {
                    addLog(
                        '✓ JavaScript program finished successfully.',
                        'success'
                    );
                } else {
                    addLog(
            ...
            if (htmlFiles.length > 0) {
                addLog(
                    `🌐 HTML file detected: ${htmlFiles[0]}`,
                    'info'
                );

                addLog(
                    'HTML preview needs a web server/project setup.',
                    'info'
                );

                addLog(
                    'For a full preview, ask @ai to create a Vite/React or Node web project.',
                    'info'
                );
            ...
            // ─────────────────────────────
            addLog(
                '⚠️ No runnable JavaScript/Node.js project found.',
                'error'
            );

            addLog(
                'Create a .js file or ask @ai to create a complete Node/React project.',
                'info'
            );
    ...
    const leaveCurrentProject =
        async () => {
            if (
                !window.confirm(
                    'Leave this project? You will lose access to its chat and files.'
                )
            ) {
                return;
            }
    ...
    return (
        <main className='h-[calc(100vh-56px)] w-full flex overflow-hidden bg-slate-900'>
            ...
            {/* LEFT CHAT */}
            <section
                className={`relative flex flex-col h-full w-full md:min-w-[290px] md:max-w-[360px] md:w-[28%] bg-slate-900 border-r border-slate-700 ${
                    mobileView === 'chat'
                        ? 'flex'
                        : 'hidden md:flex'
                }`}
            >
                <header className='flex items-center justify-between px-4 py-3 bg-slate-800 border-b border-slate-700 shrink-0'>
                    ...
                    <div className='flex items-center gap-1.5'>
                        <button
                            ...
                            className='md:hidden flex items-center gap-1 px-2.5 py-1.5 text-xs bg-slate-700 hover:bg-slate-600 text-white rounded-lg transition'
                        >
                        ...
                        <button
                            ...
                            className='flex items-center gap-1 px-2.5 py-1.5 text-xs bg-blue-600 hover:bg-blue-500 text-white rounded-lg transition'
                        >
                            <i className="ri-user-add-fill text-xs"></i>
                            Add
                        </button>
                    ...
                    {messages.map(
                        (msg, idx) => {
                            ...
                            return (
                                ...
                                >
                                    <div
                                        className={`rounded-xl px-3 py-2 text-sm max-w-full ${
                                            isAI
                                                ? 'w-full'
                                                : isOwn
                                    ...
                                    >
                                        ...
                                        {isAI ? (
                                            ...
                                            <div className='flex items-end gap-2 min-w-0'>
                                                ...
                                                <span className='text-[10px] text-slate-300/60 whitespace-nowrap self-end shrink-0'>
                                                    {formatTime(
                                                        msg
                                                    )}
                                                </span>
                ...
                <div className='p-3 border-t border-slate-700 bg-slate-800 shrink-0'>
                    <div className='flex items-end gap-2 bg-slate-700 rounded-2xl border border-slate-600 focus-within:border-blue-500 transition pl-4 pr-2 py-2'>
                        <textarea
                            ...
                            className='grow bg-transparent text-white text-sm outline-none placeholder-slate-500 resize-none custom-scroll overflow-y-auto max-h-[120px] leading-relaxed border-0 ring-0 focus:border-0 focus:ring-0 focus:outline-none shadow-none block py-1'
                        />

                        <button
                            onClick={send}
                            className='w-8 h-8 flex items-center justify-center bg-blue-600 hover:bg-blue-500 text-white rounded-lg transition shrink-0'
                        >
                ...
                <div
                    className={`absolute top-0 left-0 w-full h-full flex flex-col bg-slate-900 border-r border-slate-700 transition-transform duration-200 z-10 ${
                        isSidePanelOpen
                            ? 'translate-x-0'
                            : '-translate-x-full'
                    }`}
                >
                    ...
                    <div className='p-3 border-t border-slate-700 mt-auto'>
                        <button
                            ...
                            className='w-full flex items-center justify-center gap-2 px-4 py-2.5 text-sm text-red-400 border border-red-900/50 rounded-lg hover:bg-red-900/20 transition'
                        >
            ...
            {/* RIGHT CODE */}
            <section
                className={`flex grow h-full overflow-hidden flex-col ${
                    mobileView === 'code'
                        ? 'flex'
                        : 'hidden md:flex'
                }`}
            ...
            >
                <div className='flex grow overflow-hidden'>
                    ...
                    {/* EXPLORER */}
                    <div
                        className={`h-full flex flex-col bg-slate-900 border-r border-slate-700 transition-all duration-200 shrink-0 z-20 ${
                            explorerOpen
                                ? 'w-[200px] absolute md:relative inset-y-0 left-0 shadow-2xl md:shadow-none'
                                : 'w-9'
                        }`}
                    >
                        {explorerOpen ? (
                            <>
                                <div className='flex items-center justify-between px-2 py-2 border-b border-slate-700 shrink-0'>
                                    <span className='text-[10px] text-slate-500 uppercase tracking-widest font-medium px-1'>
                                        Explorer
                                    </span>
                                        ...
                                        <div className='flex items-center gap-2 px-3 py-1.5 bg-slate-700/50'>
                                            <i
                                                className={`text-xs ${
                                                    newItemDialog.type ===
                                                    'file'
                                                        ? 'ri-file-add-line text-slate-400'
                                                        : 'ri-folder-add-line text-amber-400'
                                                }`}
                                            ></i>
                    ...
                    {/* CODE COLUMN */}
                    <div
                        className='flex flex-col grow h-full overflow-hidden min-w-0'
                        ref={codeColumnRef}
                    >
                        {/* TABS */}
                        <div className='flex items-center justify-between bg-slate-900 border-b border-slate-700 shrink-0'>
                            <button
                                ...
                                className='md:hidden flex items-center justify-center px-3 py-2.5 text-slate-400 hover:text-white border-r border-slate-700 shrink-0'
                            >
                                ...
                                {openFiles.map(
                                    file => (
                                        <div
                                            ...
                                            className={`flex items-center gap-2 px-4 py-2.5 text-xs cursor-pointer border-r border-slate-700 shrink-0 transition ${
                                                currentFile ===
                                                file
                                                    ? 'bg-slate-800 text-white border-t-2 border-t-blue-500'
                                                    : 'text-slate-400 hover:bg-slate-800 bg-slate-900'
                                            }`}
                                        >
                            ...
                            <div className='flex items-center gap-2 px-3 shrink-0'>
                                ...
                                <button
                                    ...
                                    className={`flex items-center gap-1.5 px-4 py-1.5 text-xs rounded-lg transition text-white font-medium ${
                                        isInstalling
                        ...
                        {showTerminal && (
                            <div
                                ...
                                className='relative border-t border-slate-700 bg-slate-950 flex flex-col shrink-0'
                            >
                                <div
                                    onMouseDown={
                                ...
                                <div
                                    ...
                                    className='flex-1 overflow-y-auto p-3 font-mono custom-scroll'
                                >
                                    {terminalLogs.length ===
                                    0 ? (
                                        ...
                                        terminalLogs.map(
                                            (
                                                ...
                                                <div
                                                    ...
                                                    className={`text-xs leading-relaxed ${logColor[log.type]}`}
                                                >
                                ...
                                searchResults.length ===
                                    ...
                                    <div className='flex flex-col items-center gap-2 py-4 text-center'>
                                        ...
                                        {isValidEmailFormat(
                                            ...
                                            <>
                                                <p className='text-xs text-slate-600'>
                                                    This user is not on CodeForge AI yet
                                                </p>
                                ...
                                u => (
                                    ...
                                    >
                                        ...
                                        {selectedUserId.has(
                                            ...
                                            <i className='ri-check-fill text-blue-400 ml-auto'></i>
                                        )}
                                    </div>
                                )
                            )}
